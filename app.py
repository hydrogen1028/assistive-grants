...
def build_photo_index():
    ...
    for sub in IMAGES_DIR.iterdir():
        if sub.is_dir():
            ...
            if files:
                idx.setdefault(dev, [])
                idx[dev] += [str(p) for p in sorted(files)]
    # B: root files with prefix "<id>-"
    for p in IMAGES_DIR.glob("*"):
        if p.is_file() and p.suffix.lower() in allow and "-" in p.stem:
            dev = p.stem.split("-")[0]
            idx.setdefault(dev, [])
            idx[dev].append(str(p))
    # dedupe
...
def cite_tag():
    return "<span class='chip'>資料來源</span>"
...
def collect_sources(d: dict):
    ...
    for doc in d.get("documents", []) or []:
        url = doc.get("url")
        if url and all(url != x.get("url") for x in out):
            out.append({"label": doc.get("label", "文件"), "url": url, "note": ""})
    # citySpecifics
    for c in d.get("citySpecifics", []) or []:
        url = c.get("sourceUrl")
        if url and all(url != x.get("url") for x in out):
            out.append({"label": f"{c.get('city','—')} {c.get('program','')} 公告", "url": url, "note": "地方政府公告"})
    # generic sources (if any)
    for s in d.get("sources", []) or []:
        url = s.get("url")
        if url and all(url != x.get("url") for x in out):
            out.append({"label": s.get("label", "來源"), "url": url, "note": s.get("note", "")})
    return out
...
def build_device_pdf(d: dict) -> bytes:
    ...
    if img:
        try:
            ...
            neww = iw * scale
            newh = ih * scale
            c.drawImage(ImageReader(img), x, y - newh, width=neww, height=newh, preserveAspectRatio=True, mask="auto")
            y -= newh + 10
        except Exception:
            pass
    ...
    line(f"金額上限：{format_currency(cap)}")
    line(f"補助比例：{percent(ratio)}")
    ...
    if d.get("citySpecifics"):
        y -= 5
        line("各縣市差異：", size=14, leading=18)
        for cty in d["citySpecifics"]:
            cap2 = (cty.get("funding") or {}).get("amountCap")
            ratio2 = (cty.get("funding") or {}).get("ratioCap")
    ...
    if srcs:
        for i, s in enumerate(srcs, 1):
            line(f"{i}. {s.get('label','來源')}  {s.get('url','')}", size=10, leading=12)
    else:
        line("（尚未設定來源）", size=10)
...
def render_list_view():
    for d in filtered:
        with st.container(border=True):
            ...
            with c1:
                img_path = pick_best_image(d.get("photos") or [], 800)
                if img_path:
